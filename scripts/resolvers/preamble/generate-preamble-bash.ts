...
export function generatePreambleBash(ctx: TemplateContext): string {
  ...
  const isCodex = ctx.host === 'codex';
  const projectInstructions = isCodex ? 'AGENTS.md' : 'CLAUDE.md';
  const vendoredSkillRoot = isCodex ? '.agents/skills/gstack' : '.claude/skills/gstack';
  const conductorDetection = isCodex ? '' : `# Conductor host: AskUserQuestion is unreliable here (native disabled, MCP
...
if [ "$_SESSION_KIND" != "headless" ] && { [ -n "\${CONDUCTOR_WORKSPACE_PATH:-}" ] || [ -n "\${CONDUCTOR_PORT:-}" ]; }; then
  ...
  const planModeDetection = isCodex
...
_HAS_ROUTING="no"
if [ -f ${projectInstructions} ] && grep -q "## Skill routing" ${projectInstructions} 2>/dev/null; then
  _HAS_ROUTING="yes"
fi
